...
export async function fetchViaScraperProxy(url, options = {}) {
  ...
  if (scrapeDoToken) {
    try {
      ...
      console.error(`[Proxy] Scrape.do failed: ${error.message}. Trying ScrapingAnt fallback...`);
    }
  }
  ...
  if (scrapingAntKey) {
    try {
      ...
      console.error(`[Proxy] ScrapingAnt failed: ${error.message}. Trying next fallback...`);
    }
  }
