...
export async function getResolutionsWithSize(resolutionsMap) {
  ...
  const promises = entries.map(async ([resName, url]) => {
    let size = null;
    try {
      const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
      const response = await fetchViaScriptProxy(url, {
        headers: { 'User-Agent': USER_AGENT },
        next: { revalidate: 3600 } // cache size responses for 1 hour
      });
      if (response.ok) {
        const html = await response.text();
        // Prevent parsing fake sizes from redirected Rogmovies/Vegamovies pages
        if (!html.includes('Rogmovies') && !html.includes('vegamovies')) {
      ...
      console.error(`[Stream Resolver] Error fetching size for resolution ${resName}:`, e);
    }
    result[resName] = {
      url,
      size: size || 'Size N/A'
